defmodule VelocyPack.Encode do
  ...
  for i <- 8..1, do:
    defp encode_integer(value) when value < -power_of_2(unquote((i - 1) * 8 - 1)),
      do: {[<<unquote(0x1f + i), value::little-unsigned-size(unquote(i * 8))>>], unquote(i) + 1}

  # positive integers
  for i <- 1..8, do:
    defp encode_integer(value) when value < power_of_2(unquote(i * 8)),
      do: {[<<unquote(0x27 + i), value::little-unsigned-size(unquote(i * 8))>>], unquote(i) + 1}
  ...
  defp encode_list(value, {true, _} = opts) do # compact array
    {iodata, total_size, count} = value |> Enum.reduce({[], 0, 0},
  ...
  defp encode_list([head | tail], opts) do
    ...
    {iodata, offsets, total_size, count, equal_sizes} =
      tail
      |> Enum.reduce({[head], [0], head_size, 1, true},
  ...
  for i <- 1..4 do
    defp encode_list_without_index_table(iodata, total_size) when total_size < power_of_2(unquote(i * 8)) do
      total_size = total_size + 1 + power_of_2(unquote(i - 1))
      {[<<unquote(0x01 + i), total_size::integer-unsigned-little-unit(8)-size(power_of_2(unquote(i - 1)))>> | iodata], total_size}
    end
  end
  ...
  defp encode_list_with_index_table(iodata, count, offsets, total_size) do
    cond do
      count < power_of_2(8) and total_size + count + 3 < power_of_2(8) ->
        ...
        {[<<0x06, total_size::unsigned-size(8), count::unsigned-size(8)>>, iodata, index_table(offsets, 3, 1)], total_size}

      count < power_of_2(16) and total_size + count * 2 + 5 < power_of_2(16) ->
        total_size = total_size + count * 2 + 5
        {[<<0x07, total_size::unsigned-little-size(16), count::unsigned-little-size(16)>>, iodata, index_table(offsets, 5, 2)], total_size}

      count < power_of_2(32) and total_size + count * 4 + 9 < power_of_2(32) ->
        total_size = total_size + count * 4 + 9
        {[<<0x08, total_size::unsigned-little-size(32), count::unsigned-little-size(32)>>, iodata, index_table(offsets, 9, 4)], total_size}

      true ->
        total_size = total_size + count * 8 + 17
        {[<<0x09, total_size::unsigned-little-size(64), count::unsigned-little-size(64)>>, iodata, index_table(offsets, 17, 8)], total_size}
    end
  end
  ...
  defp encode_map(value, {_, true} = opts) do # compact object
  ...
  defp encode_map_with_index_table(iodata, count, offsets, total_size) do
    cond do
      count == 1 and total_size + 3 < power_of_2(8) ->
        ...
        {[<<0x0b, total_size::unsigned-size(8), 1::unsigned-size(8)>> | iodata], total_size}

      count < power_of_2(8) and total_size + count + 3 < power_of_2(8) ->
        total_size = total_size + count + 3
        {[<<0x0b, total_size::unsigned-size(8), count::unsigned-size(8)>>, iodata, index_table(offsets, 3, 1)], total_size}

      count < power_of_2(16) and total_size + count * 2 + 5 < power_of_2(16) ->
        total_size = total_size + count * 2 + 5
        {[<<0x0c, total_size::unsigned-little-size(16), count::unsigned-little-size(16)>>, iodata, index_table(offsets, 5, 2)], total_size}

      count < power_of_2(32) and total_size + count * 4 + 9 < power_of_2(32) ->
        total_size = total_size + count * 4 + 9
        {[<<0x0d, total_size::unsigned-little-size(32), count::unsigned-little-size(32)>>, iodata, index_table(offsets, 9, 4)], total_size}

      true ->
        total_size = total_size + count * 8 + 17
        {[<<0x0e, total_size::unsigned-little-size(64)>>, iodata, index_table(offsets, 9, 8), <<count::unsigned-little-size(64)>>], total_size}
    end
  end
  ...
  defp compact_size(size) do
    ...
    final_size = if bytes == calc_compact_bytes(final_size) do
      final_size
    else
