defmodule VelocyPack.DecoderTest do
  ...
  describe "parse" do
    ...
    test "date" do
      assert decode!(<<0x1c, 0, 83, 115, 5, -114, 0, 0, 0>>) == DateTime.from_unix!(609976800000, :milliseconds)
    end
    ...
    test "int" do
      ...
      assert decode!(<<0x27, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f>>) == 9223372036854775807
      assert decode!(<<0x28, 0xff>>) == 255
      assert decode!(<<0x29, 0xff, 0xff>>) == 65535
      assert decode!(<<0x2a, 0xff, 0xff, 0xff>>) == 16777215
      assert decode!(<<0x2b, 0xff, 0xff, 0xff, 0xff>>) == 4294967295
      assert decode!(<<0x2c, 0xff, 0xff, 0xff, 0xff, 0xff>>) == 1099511627775
      assert decode!(<<0x2d, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff>>) == 281474976710655
      assert decode!(<<0x2e, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff>>) == 72057594037927935
      assert decode!(<<0x2f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff>>) == 18446744073709551615
    end

    test "short string" do
      assert decode!(<<0x4b, 72, 97, 108, 108, 111, 32, 87, 101, 108, 116, 33>>) == "Hallo Welt!"
      assert decode!(<<0x4c, 72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100, 33>>) == "Hello World!"
    end
    ...
    test "binary" do
      ...
      assert decode!(<<0xc6, 9, 0, 0, 0, 0, 0, 0, 49, 50, 51, 52, 53, 54, 55, 56, 57>>) == expected
      assert decode!(<<0xc7, 9, 0, 0, 0, 0, 0, 0, 0, 49, 50, 51, 52, 53, 54, 55, 56, 57>>) == expected
    end
    ...
    test "array with index table" do
      ...
      assert decode!(<<0x07, 0x0e, 0x00, 0x03, 0x00, 0x31, 0x32, 0x33, 0x05, 0x00, 0x06, 0x00, 0x07, 0x00>>) == expected
      assert decode!(<<0x08, 0x18, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x31, 0x32, 0x33, 0x09,
                      0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00>>) == expected
      assert decode!(<<0x09, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x31, 0x32, 0x33, 0x09, 0x00, 0x00, 0x00,
    ...
    test "array without index table" do
      ...
      assert decode!(<<0x05, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x31, 0x32, 0x33>>) == expected

      assert decode!(<<0x02, 0x0b, 0x1b, 00::little-unsigned-size(64)>>) == [0.0]
      assert decode!(<<0x02, 0x0b, 0x1c, 00::little-unsigned-size(64)>>) == [DateTime.from_unix!(0, :milliseconds)]
    end
    ...
    test "nested arrays" do
      ...
      assert decode!(<<0x02, 0x0c, 0x02, 0x05, 0x31, 0x32, 0x33, 0x02, 0x05, 0x31, 0x32, 0x33>>) == expected
    end

    test "array with nested compact array" do
      expected = [[1, 2, 3], [1, 2, 3 ]]
      assert decode!(<<0x02, 0x0e, 0x13, 0x06, 0x31, 0x32, 0x33, 0x03, 0x13, 0x06, 0x31, 0x32, 0x33, 0x03>>) == expected
    end
    ...
    test "compact object" do
      ...
      assert decode!(<<0x14, 0x10, 0x41, 0x61, 0x28, 0x0c, 0x41, 0x62, 0x1a, 0x41, 0x63, 0x43, 0x78, 0x79,
                      0x7a, 0x03>>) == expected
    end

    test "array of objects" do
      expected = [%{"a" => 12, "b" => true, "c" => "xyz"}, %{"a" => 12, "b" => true, "c" => "xyz"}]
      assert decode!(<<0x13, 0x23, 0x14, 0x10, 0x41, 0x61, 0x28, 0x0c, 0x41, 0x62, 0x1a, 0x41, 0x63, 0x43, 0x78, 0x79,
    ...
    test "object 1 byte offset" do
      ...
      assert decode!(<<0x0b, 0xe9, 0x05, 0x41, 0x30, 0x0b, 0x2b, 0x05, 0x41, 0x30, 0x44, 0x74, 0x65, 0x73, 0x74, 0x41,
    ...
    test "object 2 bytes offset" do
      ...
      assert decode!(<<0x0c, 0x6f, 0x03, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x41,
